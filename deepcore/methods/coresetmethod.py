class CoresetMethod(object):
    ...
    def __init__(self, dst_train, args, fraction=0.5, random_seed=None, **kwargs):
        ...
        self.num_classes = len(dst_train.classes)  # Get the number of classes in the training dataset
        ...
        self.args = args  # Save the arguments object (may contain other configuration options)
        
        self.n_train = len(dst_train)  # Get the total number of samples in the training dataset
        self.coreset_size = round(self.n_train * fraction)  # Calculate the number of samples to be selected based on the fraction
