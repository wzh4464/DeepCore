...
directories = [
    "results/flip_oti_1712_40",
    "results/flip_oti_1713_30",
    "results/flip_oti_1714_20", 
    "results/flip_oti_1715_10"
]
...
for directory in directories:
    if os.path.exists(directory):
        try:
            ...
            plt.tight_layout()
            plt.savefig(f'oti_distribution_{stats["flip_percentage"]}pct.png')
            plt.close()
...
if results:
    ...
    plt.title('OTI Detection Rate vs Flip Percentage')
    plt.grid(True)
    plt.xticks(summary_df["flip_percentage"])
    plt.savefig('oti_detection_rate.png')
    plt.close()
else:
