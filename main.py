...
def main():
    ...
    # Set up batch sizes and create directories if needed
    ...
    if args.resume != "":
        try:
            ...
            # Handle case where checkpoint only contains subset information
            try:
    ...
    for exp in range(start_exp, args.num_exp):
        # Set up checkpoint name
        if args.save_path != "":
            checkpoint_name = "{dst}_{net}_{mtd}_exp{exp}_epoch{epc}_{dat}_{fr}_".format(
                dst=args.dataset, net=args.model, mtd=args.selection, dat=datetime.now(),
                exp=start_exp, epc=args.epochs, fr=args.fraction)
        ...
        # Set up models for cross-architecture experiments
