...
def calculate_l2_distance(params1, params2):
    ...
    return sum(
        torch.norm(params1[name] - params2[name]).item()
        for name in params1
        if name in params2
    )
...
def gpu_worker(gpu_id, split_data, initial_params, best_params, return_dict, epoch):
    ...
    for i in tqdm(
        range(len(split_data["parameters"])),
        desc=f"GPU {gpu_id} - Epoch {epoch}",
        position=gpu_id,
    ):
        ...
        if i == 0:
            ...
            prev_params = {
                k: v.to(device)
                for k, v in split_data["parameters"][i - 1]["params"].items()
            }

        current_params = {
            k: v.to(device) for k, v in split_data["parameters"][i]["params"].items()
        }
...
def gpu_worker_epoch_split(
    gpu_id, save_path, epochs, return_dict, initial_params, best_params
):
    """Each GPU handles its assigned epochs and computes the scores."""
    device = torch.device(f"cuda:{gpu_id}")
    best_params = {k: v.to(device) for k, v in best_params.items()}
    initial_params = {k: v.to(device) for k, v in initial_params.items()}
    ...
    for epoch in epochs:
        ...
        for i in tqdm(
            ...
            if i == 0 and epoch == min(epochs):
                ...
                prev_params = {
                    k: v.to(device)
                    for k, v in load_single_epoch_data(
                        os.path.join(save_path, f"epoch_{epoch-1}_data.pkl")
                    )["parameters"][-1]["params"].items()
                }
            else:
                prev_params = {
                    k: v.to(device) for k, v in parameters[i - 1]["params"].items()
                }

            current_params = {
                k: v.to(device) for k, v in parameters[i]["params"].items()
            }

            prev_distance = calculate_l2_distance(prev_params, best_params, device)
            current_distance = calculate_l2_distance(
...
def calculate_gpu_scores(
    ...
    if mode == "gpu_split":
        ...
        initial_params = calculate_gpu_worker_scores(
            save_path,
            num_gpus,
            return_dict,
            best_params,
        ...
        compute_gpu_scores_epoch_split(
            save_path,
            num_epochs,
            num_gpus,
            return_dict,
...
def calculate_gpu_worker_scores(
    save_path, num_gpus, return_dict, best_params, initial_params, total_scores, epoch
):
...
def compute_gpu_scores_epoch_split(
    save_path,
    num_epochs,
    num_gpus,
    return_dict,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Calculate OTI scores with different GPU processing modes."
    )
    parser.add_argument(
        "--save_path", type=str, required=True, help="Path to saved model parameters."
    )
    parser.add_argument(
        "--num_epochs", type=int, required=True, help="Number of epochs to process."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--mode",
        type=str,
        choices=["gpu_split", "epoch_split"],
        required=True,
        help="Mode of processing: 'gpu_split' or 'epoch_split'.",
    )
