...
from experiment.experiment_utils import initialize_logging_and_datasets, initialize_boundary_exp
...
def run(args, checkpoint, start_exp, start_epoch):
    ...
    for exp in range(start_exp, args.num_exp):
        ...
        all_detection_rates.append(len(found_boundary_indices) / len(boundary_indices))
    ...
    average_detection_rate = np.mean(all_detection_rates)
    average_accuracy = np.mean(all_accuracies)
    logger.info(f"平均检测率: {average_detection_rate:.4f}")
    logger.info(f"平均准确率: {average_accuracy:.4f}")
...
def initialize_boundary_exp(args, seed):
    ...
    return (
        ...
        boundary_indices,
        permuted_indices,
        boundary_dataset.get_boundary_selection_from(),
    )
